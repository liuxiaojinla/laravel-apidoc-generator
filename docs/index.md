...
```bash
php artisan vendor:publish --provider="Xin\ApiDoc\ApiDocGeneratorServiceProvider" --tag=apidoc-config
```
...
```php
$app->register(\Xin\ApiDoc\ApiDocGeneratorServiceProvider::class);
```
